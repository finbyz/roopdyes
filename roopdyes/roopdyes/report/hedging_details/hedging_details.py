...
class HedgingDetailsReport(object):
	...
	def get_data(self):
		...
		so_data = frappe.db.sql("""
			SELECT name as document, transaction_date as date, customer, currency, 
				(grand_total - (advance_paid / conversion_rate)) as total_amount, conversion_rate as rate, 
				base_grand_total as inr_amount, delivery_date, status, "Sales Order" as 'document_type'
			from `tabSales Order`
		...
		SELECT name as document, posting_date as date, customer, currency, 
				outstanding_amount as total_amount, conversion_rate as rate, 
				(outstanding_amount * conversion_rate) as inr_amount, due_date as "delivery_date", status , "Sales Invoice" as 'document_type'
			from `tabSales Invoice`
			where docstatus = 1 and status != 'Paid'
				and amount_hedged < grand_total and currency {conditions}
			order by due_date """.format(conditions=conditions), currency, as_dict=1)

		self.data = so_data + si_data
